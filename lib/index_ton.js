...
const main = async () => {
    ...
    if (window.Telegram.WebApp.initData) { 
        username = await getUsernameFromInitData(window.Telegram.WebApp.initData)
    ...
    const createGameTotal = document.querySelector('#create-game-total')
    ...
    createGameBtn.addEventListener('click', async () => {
        ...
        if (f2p_checker.checked) {
            console.log(username, map, amountOfRoundsValue, amountToPlay.toString(), numberOfPlayersValue)
...
async function getUsernameFromInitData(initData) {
    ...
    const chatEncoded = params.get('chat');
    let username = null
    ...
    if (chatEncoded) {
        try {
            // Декодируем JSON-строку
            const chatDecoded = decodeURIComponent(chatEncoded);
    
            // Преобразуем строку в объект
            const chatObj = JSON.parse(chatDecoded);

            const res = await fetch('/ton/chatid', {
                method: 'POST',
                headers: {
                    'Accept': 'application/json',
                    'Content-Type': 'application/json'

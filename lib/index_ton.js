...
const main = async () => {
    ...
    createGameBtn.addEventListener('click', async () => {
        ...
        if (f2p_checker.checked) {
            const res = await fetch('/api/f2p/create', {
                method: 'POST',
                headers: {
            ...
            if (responseStatus == 200) {
                let fights = (await (await fetch('/api/f2p')).json()).fights
    ...
    const tonLoading = async () => {
        try {
            if (address) {
                ...
                let fights = (await (await fetch('/api/ton/fights')).json()).fights
                fightInterval = setInterval(async () => {
                    try {
                        fights = (await (await fetch('/api/ton/fights')).json()).fights
                ...
                let pastFights = (await (await fetch('/api/statistics/all/?' + queryStats.toString())).json())
                await pastFightsTon(pastFights, address)
                setTimeout(() => {
                    const infos = document.querySelectorAll('#pastgames-table [data-fight]')
                    ...
                    infos.forEach((info) => {
                      info.addEventListener('click', async (ev) => {
                        ...
                        await pastFightsTon(pastFights, username)
                      })
                    })
                  }, 300)
                await openFightsTon(fights, address, tonConnectUI, isTest ? contractAddressTest : contractAddress)
                await shopTon(address, isMobile, tonConnectUI, tgInitData)
                await inventoryTon(address)
            }
            await fetch('/api/ton/setaddress', {
                method: 'POST',
                headers: {
                    'Accept': 'application/json',
                    'Content-Type': 'application/json'
    ...
    setInterval(async () => {
        try {
            fights = (await (await fetch('/api/f2p')).json()).fights
            await openFightsF2P(fights, username)
        } catch (error) {
            console.log(error)
        }
    }, 7500)
    let pastFights = (await (await fetch('/api/f2p/pastfights'+`?player=${username}`)).json()).fights
    ...
    setTimeout(() => {
        const infos = document.querySelectorAll('#pastgames-table-f2p [data-fight]')
        const infoModal = document.getElementById('info_modal')
        infos.forEach((info) => {
          info.addEventListener('click', async (ev) => {
            const fightId =  ev.target.dataset.fight
    ...
    f2pClaimNFTBtn.addEventListener('click', async () => {
        if (!address) {
            ...
            document.getElementById('pending_modal').style.display = 'flex'
            const result = await fetch('/api/ton/mintnft', {
                method: 'POST',
                headers: {
                    'Accept': 'application/json',
                    'Content-Type': 'application/json'

...
document.addEventListener("DOMContentLoaded", function () {
  try {
    if (/Mobi|Android/i.test(navigator.userAgent)) { // Проверяем, является ли устройство мобильным
      if (screen.orientation && screen.orientation.lock) {
          screen.orientation.lock("portrait-primary").catch((err) => {
              console.log("Не удалось заблокировать ориентацию экрана:", err);
          });
      }
    }    
  } catch (error) {}
...
(function ($) {
  $(function () {
    $(".inner-block__head").on("click", "button:not(.active)", function () {
      ...
      // if (isOpengames) {
      //   $("#opengames, #opengames-f2p").addClass("active");
      // } 
      // // Если нажата кнопка Past Games
      // else {
      //   $("#pastgames, #pastgames-f2p").addClass("active");
      // }
    });
  });
})(jQuery);
...
window.addEventListener('load', () => {
  ...
  const openGamesF2PEvm = document.querySelector('#opengames-f2p')
  const pastGamesF2PEvm = document.querySelector('#pastgames-f2p')
  const openGamesEmptyF2PEvm = document.querySelector('#opengames-empty-f2p')

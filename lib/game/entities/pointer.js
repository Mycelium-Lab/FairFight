...
EntityPointer = ig.Entity.extend({
    ...
    update: function() {
        ...
        if (nearestRemotePlayer) {
            ...
            let angleInDegrees = (angle * 180) / Math.PI; // Перевод угла в градусы
            let radius = cameraMaxX; // Радиус для положения стрелки вокруг камеры
            targetY = cameraY + radius * Math.sin(angle); // Координата Y с учетом угла поворота

            // Определение координаты X в зависимости от угла поворота
            let centerX = cameraX + cameraMaxX / 2; // Центр по оси X для стрелки
            let circleRadius = cameraMaxX / 2; // Радиус окружности, по которой будет двигаться стрелка
            ...
            if (targetY < cameraY) {
                targetY = cameraY;
            } else if (targetY > cameraY + cameraMaxY - arrowHeight) {

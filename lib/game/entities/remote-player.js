...
.defines(function() {
  EntityRemotePlayer = ig.Entity.extend({
    ...
    setState: function(state) {
      ...
      // if (this.prevPos.x != this.newPos.x && this.prevPos.y != this.newPos.y) {
        const time = Date.now()
        // this.dx = (this.prevPos.x - state.positionData.x) / ((time - state.positionData.time) / 60); //x - this.pos.x;
        // this.dy = (this.prevPos.y - state.positionData.y) / ((time - state.positionData.time) / 60); //y - this.pos.y;
        this.dx = state.positionData.velX
        this.dy = state.positionData.velY

...
.defines(function() {
  EntityRemotePlayer = ig.Entity.extend({
    ...
    setState: function(state) {
      ...
      // Установка новой цели для позиции
      this.targetPos = {
          x: state.positionData.x,
          y: state.positionData.y
      };

      // Установка новой цели для анимации
      this.targetAnim = this.anims[state.positionData.animName];
      this.targetAnim.frame = state.positionData.frame;
      this.targetAnim.flip.x = !!state.positionData.flip;

      // Установка скорости для плавного перемещения
      // console.log('-----------------------------')
      // console.log(x, y)
    ...
    update: function() {
      if (this.stateUpdated) {
        this.stateUpdated = false;
         ...
         this.pos.x += (this.targetPos.x - this.pos.x) * this.moveSpeed;
         this.pos.y += (this.targetPos.y - this.pos.y) * this.moveSpeed;
 
         // Интерполяция анимации
         if (this.currentAnim !== this.targetAnim) {
             // Плавный переход к новой анимации
             this.currentAnim.alpha -= this.moveSpeed;
             this.targetAnim.alpha += this.moveSpeed;

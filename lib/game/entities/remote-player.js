...
.defines(function() {
  EntityRemotePlayer = ig.Entity.extend({
    ...
    UpdateLocation: function(deltaTime) {
      let xDifference = Math.abs(Math.floor(this.pos.x) - Math.floor(this.locationWaiting.x));
      let yDifference = Math.abs(Math.floor(this.pos.y) - Math.floor(this.locationWaiting.y));
      if (xDifference <= this.tolerance && yDifference <= this.tolerance) {
        this.pos = this.locationWaiting
      } else {
        this.pos = this.LerpVec2(this.locationCurrent, this.locationWaiting);

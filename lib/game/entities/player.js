...
ig.module(
  ...
  EntityPlayer = ig.Entity.extend({
    ...
    update: function() {
      ...
      if (ig.input.pressed('shoot')) {
        ...
        if (this.weaponsLeft > 0) {
          this.weaponsLeft--;
          this.setAmmunition(this.weaponsLeft);
          ig.game.playerShoot(this.currentTypeOfBullet);
          this.currentAnim = this.anims.shoot;
          setTimeout(() => {
            this.currentAnim = this.anims.idle;
          }, 400)
    ...
    useTeleport: function(teleportId, mapId) {
      ...
      if (mapId == 1) {
        this.pos.x = this.last.x > this.pos.x ? this.pos.x - 600 : this.pos.x + 600
      }
    },
    ...
    setAmmunition: function (amount) {
      ...
      try {
        const tonAmmunition = document.querySelector('#yourStats-ton-weapons')
        tonAmmunition.textContent = amount
      } catch (error) {
    ...
    setHealth: function (amount) {
      ...
      amount = amount / 10
      try {
        const tonHealth = document.querySelector("#yourStats-ton-health")
        tonHealth.textContent = amount
      } catch (error) {
  ...
  EntityProjectile = ig.Entity.extend({
    ...
    init: function(x, y, settings) {
      ...
      /* 
        ...
        type 2 = igniting
      */
     this.typeOfBullet = settings.type
      if (!settings.type || settings.type == 0) {
        this.addAnim('idle', 1, [0]);
        this.addAnim('hit', 0.2, [1, 2, 3, 4, 5], true);
      } else if (settings.type == 1) {
        this.animSheet = new ig.AnimationSheet('../media/sprites/bulletTriple.png', 32, 44), 
        this.addAnim('idle', 1, [0]);
        this.addAnim('hit', 0.2, [1, 2, 3], true);
      } else if (settings.type == 2) {
        this.animSheet = new ig.AnimationSheet('../media/sprites/bulletIgniting.png', 32, 32), 
  ...
  EntityProjectileRemote = EntityProjectile.extend({
    ...
    check: function(other) {
      if (!this.hasHit) {
        let damage = 10
        if (this.typeOfBullet == 1) damage = 20
        if (this.typeOfBullet == 2) {
          damage = 2
          let count = 0;

    ...
    setAmmunition: function (amount) {
      ...
      for (let i = 0; i < 2; i++) {
        try {
          ...
          statsValueArr.forEach(v => {
            if(v.textContent.length > 2) {
              statsValueArr.filter(v => v.textContent.length === 1).forEach(v => {
                v.style.marginRight = '10px'
              }) 
            }
            //v.textContent.length > 2
          })
          /*if(amount > 10) {
            const img = document.getElementsByClassName('you-stats-deaths_image')
            img.insertAdjacentHTML('afterend', '<div></div>');
            //document.createElement('div')ç
          }*/

...
ig.module(
  ...
  const secWhenSend = 600 //because game updates 60/sec then it's 10 sec
  let timer = 0
  ...
  EntityPlayer = ig.Entity.extend({
    ...
    init: function(x, y, settings) {
      ...
      let playerImg = localStorage.getItem('playerimage') === null ? '../media/sprites/characters/Player.png' : localStorage.getItem('playerimage') 
    ...
    update: function() {
      if (timer === secWhenSend) {
        console.log('timer')
        timer = 0
        this.connection.check(
          {
            address: Address,
            maxWeapons: this.MAX_WEAPONS,
            size: this.size,
            offset: this.offset,
            maxVel: this.maxVel,
            accelDef: this.accelDef,
            frictionDef: this.frictionDef,
            jump: this.jump,
      ...
      if (mapID == 1) {
        ...
        if(parsedY >= 740 && parsedX >= 970 && parsedX <= 1050) {
          this.currentAnim = this.anims.spawn
          this.pos.x = 1010
          this.pos.y = 5
        }
        if(parsedY >= 740 && parsedX >= 160 && parsedX <= 226) {
          this.currentAnim = this.anims.spawn
          this.pos.x = 100
          this.pos.y = 5
        }
        if(parsedY >= 570 && parsedY <= 600 && parsedX >= 490 && parsedX <= 570) {
          this.currentAnim = this.anims.spawn
          this.pos.x = 1540
          this.pos.y = 156
        }
        if(parsedY >= 570 && parsedY <= 600 && parsedX >= 1490 && parsedX <= 1570) {
          this.currentAnim = this.anims.spawn
          this.pos.x = 530
          this.pos.y = 156
        }
        if(parsedY >= 740 && parsedX >= 1760 && parsedX <= 1850) {
          this.currentAnim = this.anims.spawn
          this.pos.x = 1935
          this.pos.y = 5
        }

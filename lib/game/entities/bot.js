...
ig.module(
    ...
    EntityBot = ig.Entity.extend({
        ...
        init: async function(x, y, settings) {
            ...
            update: function() {
                if (ig.game.player) {
                  // console.log(this.grid.grid[Math.round(this.pos.x)][Math.round(this.pos.y)], 
                  // this.grid.grid[Math.round(ig.game.player.pos.x)][Math.round(ig.game.player.pos.y)])
                  let path = astar.search(
                    this.grid, 
                    this.grid.grid[Math.round(this.pos.x)][Math.round(this.pos.y)], 
                    this.grid.grid[Math.round(ig.game.player.pos.x)][Math.round(ig.game.player.pos.y)]
                  )
    ...
    astar = {
      ...
      function Graph(gridIn, options) {
        ...
        this.grid = [];
        for (var x = 0; x < gridIn[0].length; x++) {
          this.grid[x] = [];
      
          for (var y = 0, row = gridIn; y < row.length; y++) {
            var node = new GridNode(x, y, row[y][x]);
            this.grid[x][y] = node;

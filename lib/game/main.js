...
MyGame = ig.Game.extend({
  ...
  init: function() {
    ...
    setTimeout(() => {
      ...
      this.bot = this.spawnEntity(EntityBot, spawnPos.x, spawnPos.y, {mapID: MapID, image: imageRivalCharacter.src});
    }, 500)
  ...
  update: function() {
    ...
    this.parent();
    let EntityPlayerArray = ig.game.getEntitiesByType(EntityPlayer)
    let EntityBotArray = ig.game.getEntitiesByType(EntityBot)
    if (EntityPlayerArray.length && EntityBotArray.length) {
      var path = new PathFinder(EntityBotArray[0], EntityPlayerArray[0]);
      var points = path.getPath();
      // console.log(points)
      if (points[0]) {
        let playerPosition = EntityPlayerArray[0].pos
        if (
          playerPosition.x !== this.bot.previousPositionPlayer.x && playerPosition.y !== this.bot.previousPositionPlayer.y
        ) {
          // if (
          //   Math.abs(playerPosition.x - this.bot.previousPositionPlayer.x) > 200
          //   ||
          //   Math.abs(playerPosition.y - this.bot.previousPositionPlayer.y) > 200
          //   ||
          //   this.bot.previousPositionPlayer.x === 0
          // ) {
            this.bot.targetCoordinates = points
          // }
        }
        this.bot.previousPositionPlayer = {...EntityPlayerArray[0].pos}
      }
    }
...
GameRoom = ig.Class.extend({
  ...
  onJoinedRoom: function(roomInfo) {
    ...
    ig.main('#canvas', MyGame, 60, chosenMap.x * 2, chosenMap.y * 2, 3);
  },

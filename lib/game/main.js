  ...
  if (isImageCharacterLoaded && isImageRivalCharacterLoaded) {
    let MyGame = ig.Game.extend({
      ...
      init: function() {
        ...
        // this.connection.on('joined', this.onJoinedRoom, this);
      ...
      onPeerMessage: async function(message, user, peer) {
        ...
        if (!remotePlayer && message.getType() === MESSAGE_STATE) {
          ...
          remotePlayer = await this.spawnRemotePlayer(user, message.getX(), message.getY());
      ...
      spawnRemotePlayer: async function(user, x, y) {
        ...
        this.remotePlayers[user.userId] = this.spawnEntity(EntityRemotePlayer, x, y, {image: this.remotePlayersImages[user.userId], walletAddress: user.walletAddress})
    ...
    let GameRoom = ig.Class.extend({
      ...
      onJoinedRoom: function(roomInfo) {
        ...
        roomInfo.users.forEach(v => {
          MyGame.prototype.remotePlayersAddresses[`${v.userId}`] = v.walletAddress || v.user.walletAddress
        });

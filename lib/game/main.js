  ...
  if (isImageCharacterLoaded && isImageRivalCharacterLoaded) {
    let MyGame = ig.Game.extend({
      ...
      spawnRemotePlayer: function(user, x, y) {
        this.remotePlayers[user.userId] = this.spawnEntity(EntityRemotePlayer, x, y, {image: imageRivalCharacter.src, walletAddress: user.walletAddress})
      ...
      onPlayerDied: function(remotePlayer, message, user) {
        console.log(this.remotePlayersAddresses)
        if (user !== undefined) {

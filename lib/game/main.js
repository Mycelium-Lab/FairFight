    ...
    let GameRoom = ig.Class.extend({
      ...
      onJoinedRoom: async function(roomInfo) {
        ...
        for (let i = 0; i < roomInfo.users.length; i++) {
          MyGame.prototype.remotePlayersAddresses[`${roomInfo.users[i].userId}`] = roomInfo.users[i].walletAddress || roomInfo.users[i].user.walletAddress
          if (!MyGame.prototype.remotePlayersImages[roomInfo.users[i].userId]) {
            let _rivalCharacter = new URLSearchParams();

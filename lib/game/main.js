...
async function connect() {
    ...
    pair = TonWeb.utils.nacl.sign.keyPair();
    pubkey = TonWeb.utils.bytesToHex(pair.publicKey);
    privkey = TonWeb.utils.bytesToHex(pair.secretKey);
    localStorage.setItem("publicKey", pubkey);
    localStorage.setItem("secretKey", privkey);

    console.log(`Pubkey: ${pubkey} , Private: ${privkey}`, 'color:green')

    tonweb = new TonWeb();
    wallet = tonweb.wallet.create({publicKey: pair.publicKey});
    wa = await wallet.getAddress();
    address = wa.toString(true, true, true);
...
GameRoom = ig.Class.extend({
  ...
  init: function(socketUrl, walletAddress, publicKey) {
    ...
    this.roomConnection.connect(walletAddress, publicKey);
  },

...
const rawResponse = await fetch('/getinventory', {
  ...
  body: JSON.stringify({address: Address, chainid: Network.chainid})
}).catch(err => {
    console.log(err)
    return {}
});
...
MyGame = ig.Game.extend({
  ...
  init: function() {
    ...
    // player
    this.spawnPlayer();
    // ig.game.spawnEntity(EntityPointer, 240, 240)
  ...
  spawnPlayer: function() {
    ...
    this.player = this.spawnEntity(EntityPlayer, spawnPos.x, spawnPos.y, {mapID: MapID});
    this.player.health += inventory.health == null ? 0 : inventory.health * 10
    this.player.setHealth(this.player.health)
    this.player.weaponsLeft += inventory.bullets == null ? 0 : inventory.bullets
    this.player.setAmmunition(this.player.weaponsLeft)
    this.player.MAX_WEAPONS += inventory.bullets == null ? 0 : inventory.bullets
    this.player.jump += inventory.jump == null ? 0 : inventory.jump
    this.player.maxVel.x += inventory.speed == null ? 0 : inventory.speed
  },

  ...
  if (isImageCharacterLoaded) {
    let MyGame = ig.Game.extend({
      ...
      onPeerMessage: function(message, user, peer) {
        var remotePlayer = this.remotePlayers[user.userId];
        // console.log(this.remoteKilledPlayers[user.userId])
        // console.log(this.remotePlayers)
        if (!remotePlayer && message.getType() === MESSAGE_STATE && !this.remoteKilledPlayers[user.userId]) {

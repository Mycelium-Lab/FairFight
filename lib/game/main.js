...
let imageCharacterUrl = window.webkitURL.createObjectURL(player1Image);
let imageCharacter = new Image();

let imageRivalCharacterUrl = window.webkitURL.createObjectURL(player2Image);
  ...
  if (isImageCharacterLoaded && isImageRivalCharacterLoaded) {
    ...
    let GameRoom = ig.Class.extend({
      ...
      onJoinedRoom: function(roomInfo) {
        ...
        ig.main('#canvas', MyGame, 60, chosenMap.x, chosenMap.y, ig.ua.iPhone ? 4 : 3);
      },

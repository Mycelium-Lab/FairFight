  ...
  if (isImageCharacterLoaded && isImageRivalCharacterLoaded) {
    let MyGame = ig.Game.extend({
      ...
      init: function() {
        ...
        this.connectionHandlers = {
          ...
          'remote_spawn': this.onRemoteSpawn,
          'remote_setstate': this.onPlayerState,
          'shoot': this.onPlayerShoot,
          'collect_weapon': this.onRemotePlayerCollectedWeapon,
          'player_dead': this.onPlayerDead
        };
        ...
        if (MapID == 2) {
          // ig.game.spawnEntity(EntityMovingPlatform, 201, 480, {
          //   maximumX: 830,
          //   minimumX: 200,
          //   speed: 90,
          //   moving: 0
          // })
          // ig.game.spawnEntity(EntityMovingPlatform, 1759, 480, {
          //   maximumX: 1760,
          //   minimumX: 1120,
          //   speed: 90,
          //   moving: 0
          // })
          // ig.game.spawnEntity(EntityMovingPlatform, 350, 1400, {
          //   maximumY: 1450,
          //   minimumY: 900,
          //   speed: 90,
          //   moving: 1
          // })
          // ig.game.spawnEntity(EntityTrampoline, 864, 1440, { velMinus: -500 })
          // ig.game.spawnEntity(EntityTrampoline, 1120, 1440, { velMinus: -500 })
          // // ig.game.spawnEntity(EntityTrampoline, 1000, 1000, { velMinus: -500 })
          // ig.game.spawnEntity(EntityTrampoline, 636, 956, { velMinus: -500 })
          // ig.game.spawnEntity(EntityTrampoline, 1380, 950, { velMinus: -500 })
        }
        if (MapID == 3) {
      ...
      spawnPlayer: function() {
        ...
        this.connection.socket.emit('spawn_user', {
          address,
          x: spawnPos.x,
          y: spawnPos.y
        })
      ...
      onRemoteSpawn: function(user) {
        // console.log(user)
        // this.remotePlayers[user.userId]
        this.spawnRemotePlayer(user, user.x, user.y)
      },
      ...
      spawnRemotePlayer: function(user, x, y) {
        if (!this.remotePlayers[user.userId]) {
          this.remotePlayers[user.userId] =
      ...
      onPlayerDead: function(address) {
        this.remotePlayers[address].setKill()
      },
    
      onPlayerState: function(movement) {
        // console.log(movement)
        // console.log(this.remotePlayers[movement.address])
        try {
          this.remotePlayers[movement.address].setState(movement)
        } catch (error) {
          console.log(error)
        }
      },
      ...
      onPlayerShoot: function(data) {
        ig.game.spawnEntity(EntityProjectileRemote, data.x, data.y, {
          flip: data.flip,
          userId: data.address
        });
      },
    
      onRemotePlayerCollectedWeapon: function(data) {
        this.onWeaponCollected(data.weaponId);
      },
      ...
      onPlayerCollectedWeapon: function(weapon) {
        ...
        // );
        this.connection.socket.emit('collect_weapon', {
          address,
    ...
    let GameRoom = ig.Class.extend({
      ...
      onJoinedRoom: function(roomInfo) {
        ...
        //chosenMax.x * 2 - bigger camera
        //chosenMap.x / chosenMap.reduceSizeX, chosenMap.y / chosenMap.reduceSizeY
        ig.main('#canvas', MyGame, 60, chosenMap.x * 1.34, chosenMap.y *2, ig.ua.mobile ? 0.5 : 3);
      },

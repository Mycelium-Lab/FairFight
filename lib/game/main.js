...
MyGame = ig.Game.extend({
  ...
  playerDied: function(killerId) {
    this.connection.playerDied();
    if (this.connection.paymentChannel) {
      this.connection.paymentChannel.getLoseSigendState()
      this.connection.editPaymentChannel();
    }
  ...
  onPlayerDied: function(remotePlayer, message, user) {
    this.connection.playerDied();
    // this.connection.paymentChannel.getWinSigendState()
    // this.connection.editPaymentChannel();
...
window.gameRoom = new GameRoom(
  `${window.location.protocol}//` + window.location.hostname + ':8033',
  address,

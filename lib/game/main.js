...
reader.onload = function(event) {
    try {
        imageCharacter.onload = function() {
          const p1 = document.querySelector('#you-stats-skin')
          p1.style.backgroundImage = `url(${imageCharacter.src})`
          console.log(imageCharacter, imageCharacter.src)  
...
secondReader.onload = function(event) {
    try {
        imageRivalCharacter.onload = function() {
            const p2 = document.querySelector('#enemy-stats-skin')
            p2.style.backgroundImage = `url(${imageRivalCharacter.src})`
            console.log(imageRivalCharacter, imageRivalCharacter.src)  
...
const Maps = [
  {
    ...
    reduceSizeY: 1,
    minimap: {x: 200, y: 200},
    minimapPlace: {x: 0, y: 900},
    minimapPlaceMobile: {x: 0, y: 125}
  },
  {
    level: LevelMoon,
    ...
    reduceSizeY: 1.3,
    minimap: {x: 260, y: 150},
    minimapPlace: {x: 0, y: 600},
    minimapPlaceMobile: {x: 0, y: 100}
  }
]

            ...
            if (
                ...
                fetch('/balance?' + querySign.toString())
                    .then(async (res) => {
                        ...
                        internalBalance.textContent = `${data.balance.length == 0 ? ethers.utils.formatEther(battle.player1Amount.toString()) : ethers.utils.formatEther(data.balance)} `
                        ...
                        const _rounds = totalDeposit / parseInt(battle.amountForOneDeath.toString()) / 2
                        const gameEndedRounds = _rounds - parseInt(data.remainingRounds)

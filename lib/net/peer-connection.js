...
PeerConnection = Events.Emitter.extend({
  ...
  // }
  {
    url: 'turn:13.51.64.57:3478',
    urls: 'turn:13.51.64.57:3478',
    username: 'user',
    credential: 'password'
  }
  ],
  ...
  onIceConnectionStateChanged: function(event) {
    try {
      ...
      this.log('Connection state: ' + state, 'green');
  
      if (state === 'disconnected' || state === 'failed') {
        this.log('ICE connection state is ' + state + ', restarting ICE...', 'orange');
  
        this.restartIceInterval = setInterval(() => {
          if (this.peerConnection.iceConnectionState !== 'connected') {
                // Ручной перезапуск ICE
            this.peerConnection.createOffer()
            .then(offer => this.peerConnection.setLocalDescription(offer))
            .then(() => {
              this.log('New offer created, sending to remote peer', 'orange');
              this.sendSdp(this.peerUser.userId, this.peerConnection.localDescription);
            })
            .catch(error => {
              this.log('Error during ICE restart: ' + error.message, 'red');
            });
            this.log('Attempting to restart ICE...', 'orange');
            this.peerConnection.restartIce();
          } else {
            clearInterval(this.restartIceInterval);
            this.log('ICE connection state is connected.', 'green');
          }
        }, 1000);
      }
    } catch (error) {
      console.error('Error in onIceConnectionStateChanged:', error);
    }
  },

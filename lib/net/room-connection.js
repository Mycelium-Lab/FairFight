...
RoomConnection = Events.Emitter.extend({
  ...
  onUpdateBalance: function (data) {
    ...
    //   console.log(this.peers)
    //   // this.socket.to(value.id).emit('update_balance', data)
    // })
    ...
    //   internalBalance.textContent = `Your in-game balance: ${ethers.utils.formatEther(data.amount1)} ETH`
    //   enemyBalance.textContent = `Your enemy in-game balance: ${ethers.utils.formatEther(data.amount2)} ETH`
    // } else {
    //   internalBalance.textContent = `Your in-game balance: ${ethers.utils.formatEther(data.amount2)} ETH`
    //   enemyBalance.textContent = `Your enemy in-game balance: ${ethers.utils.formatEther(data.amount1)} ETH`
    // }
  },

...
.defines(function() {
    ...
    TONChannel = function(walletAddress, publicKey, isA) {
        window.document.getElementById("loadingStage").innerText = "Payment channel creation..."    
        this.isA=isA;
        ...
        return this.myWallet.getAddress().then((address) => {
            ...
            window.document.getElementById("loadingStage").innerText = "Payment channel opening..."    
            ...
            console.log('Channel opened: https://testnet.tonscan.org/address/'+this.channelAddress);
            return this;
        });
    }
    TONChannel.prototype = {
        create: async function() {
            ...
            if (this.isA) {
                try {
                    window.document.getElementById("loadingStage").innerText = "Payment channel deploying..."    
                    console.log('Deploying the channel...');
                ...
                window.document.getElementById("loadingStage").innerText = "Waiting for payment channel deploy..."    
            }
            return '';
        },
        topUp: async function() {
            window.document.getElementById("loadingStage").innerText = "Waiting topup complete..."
            const elChannel = window.document.getElementById("paymentChannel");
            elChannel.style.display = "inherit";
            elChannel.innerHTML = `Payment channel: <a target="_blank" href="https://testnet.tonscan.org/address/${this.channelAddress}">${this.channelAddress}</a>`;
            let balance;
            if (this.isA) {
        ...
        init: async function() {
            if (this.isA) {
                window.document.getElementById("loadingStage").innerText = "Payment channel initializing..."    
                ...
                window.document.getElementById("loadingStage").innerText = "Waiting for payment channel initialization..."    
        ...
        signClose: async function() {
            this.channelState.seqnoA = this.channelState.seqnoA.add(new BN(1));
            this.channelState.seqnoB = this.channelState.seqnoB.add(new BN(1));
            console.log('Signing channel close...');
            console.log('balanceA: ', this.channelState.balanceA.toString());
            console.log('balanceB: ', this.channelState.balanceB.toString());
            console.log('seqnoA: ', this.channelState.seqnoA.toString());
            console.log('seqnoB: ', this.channelState.seqnoB.toString());
        ...
        closeSigned: async function(signature) {
            ...
            await this.fromWallet.close({
                ...this.channelState,
                hisSignature: signature
            }).send(toNano('0.05'));
            // await until channel state is 0, meaning the closing is completed
            return new Promise(resolve => {
                checkStatus = async () => {
                    if (await this.channel.getChannelState() == 0) {
                        resolve();
                    } else {

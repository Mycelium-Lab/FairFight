...
export async function lootbox(address, network, signer, wrap, isMobile) {
    try {
        ...
        if (network.lootboxAddress) {
            ...
            const _allowance = network.chaindid == 23294 ? await usdtContract.connect(wrap(signer)).allowance(address, lootboxContract.address) : await usdtContract.allowance(address, lootboxContract.address)
            const _price = network.chaindid == 23294 ? await lootboxContract.connect(wrap(signer)).price() : await lootboxContract.price()
                                ...
                                tx.wait()
                                    .then(async (_waited) => {
                                        try {
                                            ...
                                            if (allowance >= price) {
                                                buyButton.textContent = 'Buy now'
                                                await buy(address, lootboxContract, usdtContract, network, wrap, signer)
                                            } else {
                                                showError({message: 'insufficient allowance'})
                                            }
                                        } catch (error) {
...
async function buy(address, lootboxContract, usdtContract, network, wrap, signer) {
    const tx = network.chaindid == 23294 ? await lootboxContract.connect(wrap(signer)).buy() : await lootboxContract.buy()

...
export async function lootbox(address, network, signer, wrap, isMobile) {
    try {
        ...
        if (network.lootboxAddress) {
            ...
            const _allowance = (network.chainid == 23294 || network.chainid == 355113) ? ethers.constants.MaxUint256 : await usdtContract.allowance(address, lootboxContract.address)
...
async function buy(address, lootboxContract, usdtContract, network, wrap, signer) {
    let tx
    if (network.chainid == 23294) {
        tx = await lootboxContract.connect(wrap(signer)).buyNative({value: price})
    } else if (network.chainid == 355113) {
        tx = await lootboxContract.buyNative({value: price})
    } else {
        await lootboxContract.buy()
    }

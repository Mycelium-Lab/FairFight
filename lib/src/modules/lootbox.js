...
export async function lootbox(address, network, signer, wrap, isMobile) {
    try {
        ...
        if (network.lootboxAddress) {
            ...
            const _allowance = network.chainid == 23294 ? ethers.constants.MaxUint256 : await usdtContract.allowance(address, lootboxContract.address)
            const _price = network.chainid == 23294 ? await lootboxContract.price() : await lootboxContract.price()
...
async function buy(address, lootboxContract, usdtContract, network, wrap, signer) {
    const tx = network.chainid == 23294 ? await lootboxContract.connect(wrap(signer)).buyNative({value: price}) : await lootboxContract.buy()

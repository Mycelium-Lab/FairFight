...
export const pastFightsFunc = async (address, contract, network, signer, isDesktop) => {
    ...
    let statistics = await fetch('/api/getstatisticsmulti?' + queryGameProps).then(async (res) => (await res.json())).catch(err => {
        console.log(err)
        return []
    })
    ...
    if(isInfoModal) {
        ...
        await addToPastFights(pastFights[index], network, signer, address, '', isInfoModal, mapIDs.length ? mapIDs[index].map : 0, statistics.length ? statistics[index] : 0)
    }
    else {
        for (let i = 0; i < pastFights.length; i++) {
            await addToPastFights(pastFights[i], network, signer, address, '', isInfoModal, mapIDs.length ? mapIDs[i].map : 0, statistics.length ? statistics[i] : 0)
        }
    }

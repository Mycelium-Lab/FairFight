...
export const pastFightsFunc = async (address, contract, network, signer, isDesktop) => {
    ...
    let pastFightsIds = []
    pastFights = pastFights
        .filter(v => {
            const isValid = v.owner !== ethers.constants.AddressZero && v.finishTime !== 0;
            if (isValid) {
                pastFightsIds.push(v.ID); 
            }
            return isValid;
        })
        .sort(compareID);
    ...
    queryGameProps.append("gameids", pastFightsIds)
    queryGameProps.append("chainid", network.chainid)
    let mapIDs = await fetch('/api/getgamespropsall?' + queryGameProps).then(async (res) => (await res.json())).catch(err => {
      console.log(err)
      return []
    })
    let statistics = await fetch('/api/statisticsmulti?' + queryGameProps).then(async (res) => (await res.json())).catch(err => {
        console.log(err)
        return []
    })
    ...
    if(isInfoModal) {
        ...
        await addToPastFights(pastFights[index], network, signer, address, '', isInfoModal, mapIDs.length ? mapIDs[index].map : 0, statistics.length ? statistics[index].statistics : [])
    }
    else {
        for (let i = 0; i < pastFights.length; i++) {
            await addToPastFights(pastFights[i], network, signer, address, '', isInfoModal, mapIDs.length ? mapIDs[i].map : 0, statistics.length ? statistics[i].statistics : [])
        }
    }
...
pastgamesBtn.addEventListener('click', () => {
    ...
    const pastGamesList = isDesktop ? document.getElementById('pastgames-list-desktop') : document.getElementById('pastgames-list').querySelector('.simplebar-content')
...
export const addToPastFights = async (fight, network, signer, address, from, isInfoModal, mapID, stats) => {
    const mapProperties = getMap(mapID)
    let pastGamesList = isDesktop ? document.getElementById('pastgames-list-desktop') : document.getElementById('pastgames-list').querySelector('.simplebar-content')

        ...
        if (alreadyExistHere == false) {
            ...
            fetch('/statistics?' + queryStatsPlayer1.toString())
                .then(async (res) => {
                    ...
                    if (amountLose1 > 0) {
                        winLoseText = `Lose: ${amountLose1 / BigInt(10**decimals)} ${token}`
                        classNameWinLose = "red"
                    } else if (amountLose1 < 0) {
                        winLoseText = `Win: ${(amountLose1 * BigInt(-1)) / BigInt(10**decimals)} ${token}`
                    } else {
                    ...
                    let winLoseText2;
                    if (amountLose2 > 0) {
                        winLoseText2 = `Lose: ${amountLose2 / BigInt(10**decimals)} ${token}`
                        classNameWinLose2 = "red"
                    } else if (amountLose2 < 0) {
                        winLoseText2 = `Win: ${(amountLose2 * BigInt(-1)) / BigInt(10**decimals)} ${token}`
                    } else {

...
export const addToPastFights = async (fight, network, signer, address, from) => {
    ...
    fetch('/statistics?' + queryStatsPlayer1.toString())
        .then(async (res) => {
            ...
            //PLAYER 1 (YOU)
            const amountLose1 = BigInt(fight.baseAmount.toString()) - BigInt(player1Stats.amount.toString())
            const amountLose2 = BigInt(fight.baseAmount.toString()) - BigInt(player2Stats.amount.toString())
            let winLoseText = '';

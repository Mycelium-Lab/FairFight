...
export const pastFightsFunc = async (address, contract, network, signer, isDesktop) => {
    ...
    if(isInfoModal) {
        ...
        await addToPastFights(pastFights[index], network, signer, address, '', isInfoModal, mapIDs.length ? mapIDs[index].map : 0)
    }
    else {
        for (let i = 0; i < pastFights.length; i++) {
            await addToPastFights(pastFights[i], network, signer, address, '', isInfoModal, mapIDs.length ? mapIDs[i].map : 0)
        }
    }
...
export const addToPastFights = async (fight, network, signer, address, from, isInfoModal, mapID) => {
    ...
    fetch('/api/statistics?' + gameStats.toString())
        .then(async (res) => {

...
export const openFightsTon = async (fights, address, tonConnectUI) => {
    ...
    if (fights.length === 0) {
      ...
      fights.forEach(async (fight) => {
        const child = await appendOpenFight(fight, tonChainId, address, tonConnectUI)
...
const appendOpenFight = async (fight, chainid = tonChainId, address, tonConnectUI) => {
    let index = fight.id
    ...
    if (fight.players.includes(address)) {
    ...
    button.addEventListener('click', async (event) => {
        ...
        if (buttonText === BUTTON_TEXT.FINISH) {
            ...
            const finishData = new Builder()
            finishData.storeUint(0xB7766AF2, 32)
            finishData.storeInt(index, 257)
            finishData.storeAddress(Address.parse(address))
            finishData.storeAddress(Address.parse('EQDeOj6G99zk7tZIxrnetZkzaAlON2YZj0aymn1SdTayohvZ'))
            finishData.storeCoins(amount);

...
export const openFightsTon = async (fights, address, tonConnectUI) => {
    ...
    if (fights.length === 0) {
      ...
      fights.forEach(async (fight, index) => {
        const child = await appendOpenFight(fight, index, tonChainId, address, tonConnectUI)
...
const appendOpenFight = async (fight, index, chainid = tonChainId, address, tonConnectUI) => {

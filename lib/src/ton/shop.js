...
export const shopTon = async (address, isMobile, tonConnectUI) => {
    /*if(!isDesktop) {
    ...
    await _shop(address, isMobile, tonConnectUI).catch(err => console.log(err))
...
const _shop = async (address, isMobile, tonConnectUI) => {
    ...
    await createShopItem(
        address, 'characters', charactersJsons, document.querySelector('#nft-shop-characters-list'), tonConnectUI
    )
    await createShopItem(
        address, 'armors', armorsJsons, document.querySelector('#nft-shop-armors-list'), tonConnectUI
    )
    await createShopItem(
        address, 'weapons', weaponsJsons, document.querySelector('#nft-shop-weapons-list'), tonConnectUI
    )
    await createShopItem(
        address, 'boots', bootsJsons, document.querySelector('#nft-shop-boots-list'), tonConnectUI
    )
    collectionChange()
...
const createShopItem = async (
    address, nftTypeName, jsons, list, tonConnectUI
) => {

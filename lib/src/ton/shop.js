...
import { lootboxTon } from './lootbox.js'
...
const shopAddress = "EQCaRsuhnrB6QIsDVD2pbXC9aPbsCQth_ZcQwv0GdJn7bC8t"
const shopAddressTest = ""
...
export const shopTon = async (address, isMobile, tonConnectUI, tgInitData) => {
    /*if(!isDesktop) {
    ...
    await _shop(address, isMobile, tonConnectUI, tgInitData).catch(err => console.log(err))
...
let ascendingPriceClicked = false
let descendingPriceClicked = false

const _shop = async (address, isMobile, tonConnectUI, tgInitData) => {
    document.querySelector('#ascending-price-svg').addEventListener('click', () => {
        if (!ascendingPriceClicked) {
            ascendingPriceClicked = true
            ...
            setTimeout(() => { ascendingPriceClicked = false }, 200)
        }
    })
    document.querySelector('#descending-price-svg').addEventListener('click', () => {
        if (!descendingPriceClicked) {
            descendingPriceClicked = true
            const ascending = document.querySelector('#ascending-price-svg g')
            const descending = document.querySelector('#descending-price-svg g')
            ...
            setTimeout(() => { descendingPriceClicked = false }, 200)
        }  
    })
    await createShopItem(
        address, 'characters', charactersJsons, document.querySelector('#nft-shop-characters-list'), tonConnectUI, tgInitData
    )
    await createShopItem(
        address, 'armors', armorsJsons, document.querySelector('#nft-shop-armors-list'), tonConnectUI, tgInitData
    )
    await createShopItem(
        address, 'weapons', weaponsJsons, document.querySelector('#nft-shop-weapons-list'), tonConnectUI, tgInitData
    )
    await createShopItem(
        address, 'boots', bootsJsons, document.querySelector('#nft-shop-boots-list'), tonConnectUI, tgInitData
    )
    collectionChange()
    await lootboxTon(address, isMobile, tonConnectUI)
}

const createShopItem = async (
    address, nftTypeName, jsons, list, tonConnectUI, tgInitData
) => {
    ...
    jsons.forEach(async (v, i) => {
        ...
        if (nftTypeName === 'boots') {
            ...
            const price = parseFloat(v.priceTon).toFixed(2)
                    ...
                    infoLeft.classList.add('left__info')
                        ...
                        if(!isDesktop) {
                           ...
                           // const infoPricingTitle = document.createElement('p')
                            ...
                            const infoPricingBuyNowDropdown = document.createElement('div')
                            const infoPricingBuyTon = document.createElement('span')
                            const infoPricingBuyAeon = document.createElement('span')
                            const infoPricingLine = document.createElement('span')
                            infoPricingLine.classList.add('line')
                            infoPricingBuyNowDropdown.classList.add('buy-now-btn__wrapper-dropdown')
                            
                            
                            
                            infoPricingBuyTon.textContent = 'Buy via Ton'
                            infoPricingBuyAeon.textContent = 'Buy via Aeon'
                            infoPricingBuyTon.id = "buy-via-ton"
                            infoPricingBuyTon.addEventListener('click', () => {
                                infoPricingBuyTon.style.backgroundColor = "var(--bs-warning)";
                                setTimeout(() => {
                                    infoPricingBuyTon.style.backgroundColor = "transparent";                                    
                                }, 250);
                            })
                            infoPricingBuyAeon.addEventListener('click', () => {
                                infoPricingBuyAeon.style.backgroundColor = "var(--bs-warning)";
                                setTimeout(() => {
                                    infoPricingBuyAeon.style.backgroundColor = "transparent";                                    
                                }, 250);
                            })
                            infoPricingBuyNowDropdown.appendChild(infoPricingBuyTon)
                            infoPricingBuyNowDropdown.appendChild(infoPricingLine)
                            infoPricingBuyNowDropdown.appendChild(infoPricingBuyAeon)
                            // if (bought[i] != 0) {
                            ...
                            infoPricingBuyNowButton.addEventListener('click', () => {
                                if (infoPricingBuyNowDropdown.style.display !== "flex") {
                                    infoPricingBuyNowDropdown.style.cssText = 'display: flex !important;';                                   
                                } else {
                                    infoPricingBuyNowDropdown.style.cssText = 'display: none !important;';
                                }
                            })
                            const eventButton = isDesktop ? infoPricingBuyNowButton : infoPricingBuyTon
                            ...
                            eventButton.addEventListener('click', async () => {
                                ...
                                const body = beginCell()
                                    ...
                                    .storeInt(BigInt(nftTypeIndex), 257)
                                    .storeInt(BigInt(i), 257)
                                    .endCell();
                                ...
                                const transaction = {
                                    ...
                                    messages: [
                                        {
                                            address: Address.parse(isTest ? shopAddressTest : shopAddress).toString(),
                                            amount: (toNano(v.priceTon) + toNano(0.05) * 2n).toString(), 
                                ...
                                if (result) {
                                    ...
                                    let checkerInterval = setInterval(async () => {
                                        try {
                                            ...
                                            if (newItem) {
                                                newItem.height = successModalImg.height
                                                newItem.width = successModalImg.width
                                                addInventoryItem(listItemId, newItem, nftTypeName, address, 0, firstEmpty, newItem.address)
                            ...
                            infoPricingBuyAeon.addEventListener('click', async () => {
                                const bodyData = {
                                    initData: tgInitData,
                                    nftType: nftTypeIndex,
                                    nftId: parseInt(i)
                                }
                                const response = await fetch('/api/aeon/sign', {
                                    method: 'POST',
                                    headers: {
                                ...
                                if (response.status == 200) {
                                    resSign = await response.json()
                                    const responseAeon = await fetch('/apihttps://sbx-crypto-payment-api.aeon.xyz/open/api/tg/payment/V2', {
                                        method: 'POST',
                                        headers: {
                                            'Accept': 'application/json',
                                            'Content-Type': 'application/json'
...
export async function toggleDressingModal(
    ...
    try {
        ...
        if (fittingModalOverlay.classList.contains('modal-overlay_active')) {
            ...
            const res = await fetch('/apitryon', {
                method: 'POST',
                headers: {
                    'Accept': 'application/json',
                    'Content-Type': 'application/json'

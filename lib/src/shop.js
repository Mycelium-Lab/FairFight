...
const createShopItem = async (
    ...
    const usdtContractCall = (contract) => {
        tryAgainButton.addEventListener('click', () => {
            confirmModal.style.display = 'flex'
            errorModal.style.display = 'none'
            usdtContractCall(contract)
        })
        contract.approve(shopContract.address, ethers.constants.MaxUint256)
            .then(async (tx) => {
                ...
                tx.wait()
                    .then(() => {
                        ...
                        successModalImg.src = `/media/tokens/black/usdt.png`
                        ...
                        infoPricingBuyNowButton.addEventListener('click', () => {
                            ...
                            if (network.chainid == 23294) {
                                if (infoPricingBuyNowButton.textContent === `Buy now`) {
                                    ...
                                    usdtContractCall(usdtContract.connect(wrap(signer)))
                                }
                            } else {
                                if (infoPricingBuyNowButton.textContent === `Buy now`) {
                                    confirmModal.style.display = 'flex'

...
const _shop = async (address, shopContract, characters, armors, weapons, boots, network, signer, wrap, isMobile) => {
    ...
    const allowance = (network.chainid == 23294 || network.chainid == 355113) ? ethers.constants.MaxUint256 :  await usdtContract.allowance(address, shopContract.address)
                           ...
                           // const infoPricingTitle = document.createElement('p')
                            ...
                            infoPricingBuyNowButton.addEventListener('click', async () => {
                                ...
                                if (network.chainid == 23294) {
                                    ...
                                    if (infoPricingBuyNowButton.textContent === `Buy now`) {
                                        confirmModal.style.display = 'flex'
                                        shopContract.buy(nftContract.address, usdtContract.address, i, network.chainid == 355113 ? {value: (parseInt(v.price * 10**18)).toString(), nonce: await signer.getTransactionCount()} : {})
                                            .then(async (tx) => {
                                                confirmModal.style.display = 'none'
                                                progressModal.style.display = 'flex'
                                                tx.wait()
                                            ...
                                            .then((tx) => {
                                                ...
                                                tx.wait()
                                                    .then((_waited) => {
                                                        ...
                                                        progressModal.style.display = 'none'
                                                        confirmModal.style.display = 'flex'
                                                            shopContract.buy(nftContract.address, usdtContract.address, i, network == 355113 ? {value: (parseInt(v.price * 10**18)).toString()} : {} )
                                                                .then(async (tx) => {
                                                                    confirmModal.style.display = 'none'
                                                                    progressModal.style.display = 'flex'
                                                                    tx.wait()

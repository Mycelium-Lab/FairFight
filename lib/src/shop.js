...
const _shop = async (address, shopContract, characters, armors, weapons, boots, network, signer, wrap) => {
    ...
    await createShopItem(
        address, allowance, shopContract, usdtContract, 'USDT', characters, 'characters', charactersJsons, document.querySelector('#nft-shop-characters-list'), network, signer, wrap
    )
    await createShopItem(
        address, allowance, shopContract, usdtContract, 'USDT', armors, 'armors', armorsJsons, document.querySelector('#nft-shop-armors-list'), network, signer, wrap
    )
    await createShopItem(
        address, allowance, shopContract, usdtContract, 'USDT', weapons, 'weapons', weaponsJsons, document.querySelector('#nft-shop-weapons-list'), network, signer, wrap
    )
    await createShopItem(
        address, allowance, shopContract, usdtContract, 'USDT', boots, 'boots', bootsJsons, document.querySelector('#nft-shop-boots-list'), network, signer, wrap
    )
    collectionChange()
}

const createShopItem = async (
    address, allowance, shopContract, usdtContract, symbol, nftContract, nftTypeName, jsons, list, network, signer, wrap
) => {
    ...
    for (let i = 0; i < jsons.length; i++) {
        ...
        const collection = nftTypeName === 'characters' ? 'Character' : jsons[i].attributes.find(v => v.trait_type === 'Class').value
        let healthBonus
        let speedBonus
        let jumpBonus
        let bulletsBonus
        ...
        if (nftTypeName === 'armors') {
            healthBonus = jsons[i].attributes[2].value
            listItemId = "#left-inventory-armors-list"
        }
        if (nftTypeName === 'weapons') {
            bulletsBonus = jsons[i].attributes[2].value
            listItemId = "#left-inventory-weapons-list"
        }
        if (nftTypeName === 'boots') {
            speedBonus = jsons[i].attributes[2].value
            jumpBonus = jsons[i].attributes[3].value
        ...
        listItem.classList.add('list-items__item', `list-items__item_${collection.toLowerCase()}`)
            ...
            itemLeft.classList.add('item__left')
                ...
                infoLeft.classList.add('left__info')
                    ...
                    infoPricing.classList.add('info__pricing')
                        ...
                        infoPricingBuyNowButton.textContent = BigInt(prices[i]) > BigInt(allowance) ? 'Approve' : 'Buy now'
                        if (bought[i] != 0) {
                                            ...
                                            tx.wait()
                                                .then(() => {
                                                    ...
                                                    addInventoryItem(listItemId, {
                                                        ...
                                                        name: jsons[i].name ? jsons[i].name : 'Name',
                                                        attributes: jsons[i].attributes,
                                                        collection
                                ...
                                if (infoPricingBuyNowButton.textContent === `Buy now`) {
                                    ...
                                    shopContract.buy(nftContract.address, usdtContract.address, i)
                                        .then(async (tx) => {
                                            ...
                                            tx.wait()
                                                .then(() => {
                                                    __allowance -= BigInt(prices[i].toString())
                                                    updateAllowance()
                                                    showGoToInventoryBtn(true)
                                                    progressModal.style.display = 'none'
                                                    infoPricingBuyNowButton.style.color = '#393939'
            ...
            itemRight.classList.add('item__right')
                ...
                itemRightDescription.textContent = jsons[i].description
...
const updateAllowance = () => {
    ...
    let armorsItems = Array.from(armors.querySelectorAll('.list-items__item'))
    let bootsItems = Array.from(boots.querySelectorAll('.list-items__item'))
    charactersItems.forEach(item => _updateAllowance(item, __allowance))
    weaponsItems.forEach(item => _updateAllowance(item, __allowance))
    armorsItems.forEach(item => _updateAllowance(item, __allowance))
    bootsItems.forEach(item => _updateAllowance(item, __allowance))
}

const _updateAllowance = (item, allowance) => {
    try {
        const price = item.getAttribute('data-price')
        const weiPrice = BigInt(ethers.utils.parseEther(price).toString())
        const button = item.querySelector('button')
        if (allowance >= weiPrice) {

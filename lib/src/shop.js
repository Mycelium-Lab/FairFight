...
export const shop = async (address, network, signer, wrap, isMobile) => {
    ...
    const shopContract = signer ? new ethers.Contract(network.shopAddress, shopAbi, signer) : null
    const characters =signer ?  new ethers.Contract(network.charactersAddress, nftAbi, signer) : null
    const armors = signer ? new ethers.Contract(network.armorsAddress, nftAbi, signer) : null
    const boots = signer ? new ethers.Contract(network.bootsAddress, nftAbi, signer) : null
    const weapons = signer ? new ethers.Contract(network.weaponsAddress, nftAbi, signer) : null
    shopSwitcher()
...
const _shop = async (address, shopContract, characters, armors, weapons, boots, network, signer, wrap, isMobile) => {
    ...
    let usdt = _tokens.list.find(v => v.symbol == 'USDT' || v.symbol == 'weUSDT' || v.symbol == 'tUSDT' || v.symbol == 'USDC')
    ...
    //MOCK
    const allowance = (network.chainid == 23294 || network.chainid == 355113 || network.chainid == 1440002 || network.chainid == 31338) ? ethers.constants.MaxUint256 :  await usdtContract.allowance(address, shopContract.address)
                                    ...
                                    if (infoPricingBuyNowButton.textContent === `Buy now`) {
                                        ...
                                        shopContract.buy(nftContract.address, usdtContract.address, i, (network.chainid == 355113 || network.chainid == 1440002) ? {value: (parseInt(v.price * 10**18)).toString(), nonce: await signer.getTransactionCount()} : {})
                                            ...
                                            .catch(err => {
                                                ...
                                                err.message = changeErrMessage(err.data ? err.data : err)
                                                errorModalText.textContent = err.message
                                                confirmModal.style.display = 'none'
                                                approveModal.style.display = 'none'
                                                errorModal.style.display = 'flex'

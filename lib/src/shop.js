...
export const shop = async (address, network, signer, wrap) => {
    ...
    shopSwitcher()
    await charactersShop(address, characterShopContract, network, signer, wrap).catch(err => console.log(err))
}
    ...
    for (let i = 0; i < charactersBlockBox.length; i++) {
        ...
        if (item.id.includes('character')) {
            ...
            if (characterBought != 0) {
                ...
                const characterDataUrl = await characterShopContract.characterURIs(characterID).catch(err => {
                    console.log(err)
                    return ''
                })
                const characterPrice = await characterShopContract.allowedTokens(usdt.address, characterID).catch(err => {
                    console.log(err)
                    return 0
                })
                const characterDataFetched = await fetch(characterDataUrl).catch(err => {
                    console.log(err)
                    return ''
                })
                const characterData = await characterDataFetched.json().catch(err => {
                    console.log(err)
                    return {}
                })
                ...
                item.addEventListener('click', async () => {
                    const allowance = await usdtContract.allowance(address, characterShopContract.address).catch(err => {
                        console.log(err)
                        return 0
                    })
                    characterModalImg.src = `/media/characters/${parseInt(characterID)+1}.png`

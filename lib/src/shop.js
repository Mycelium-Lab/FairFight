...
const shopSwitcher = () => {
    ...
    document.querySelector('#nft-shop-2__tab-5').addEventListener('click', () => {
        ...
        nftShopArmorsList.style.display = 'none'
        nftShopBootsList.style.display = 'none'
        nftBoxesBootsList.style.display = 'block'
    })
}
        ...
        function displayItemChange(items) {
            for(let i = 0; i < items.length; i++) {
                ...
                if (selectedCollections.length === 0) {
                    items[i].style.display = isDesktop ? "flex" : "inline-block";
                } else {
                    if (selectedCollections.includes(collection)) {
                        items[i].style.display = isDesktop ? "flex" : "inline-block"; // Показываем персонажа
                    } else {
...
let nftItemModal = document.querySelector('#nft-item-modal')

/*const showNftItemModal = () => {
    nftItemModal.style.display = 'flex'
    nftItemModal.style.alignItems = 'center'
}*/


/*const nftInfoClose = document.querySelector('#btn-close-nft-item-modal')
nftInfoClose.addEventListener('click', closeShopItemModal)


function closeShopItemModal ()  {
    nftItemModal.style.display = 'none'
}*/
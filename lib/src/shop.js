...
const createShopItem = async (
    ...
    for (let i = 0; i < jsons.length; i++) {
        ...
        let jumpBonus
        let bulletsBonus
        if (nftTypeName === 'armors') healthBonus = jsons[i].attributes[2].value
        if (nftTypeName === 'weapons') bulletsBonus = jsons[i].attributes[3].value
        ...
        listItem.classList.add('list-items__item', `list-items__item_${collection.toLowerCase()}`)
            ...
            itemRight.classList.add('item__right')
                ...
                if (bulletsBonus) itemRightProps.appendChild(nftCharacteristic('/media/svg/props-items/attack.svg', 'Bullets', bulletsBonus))

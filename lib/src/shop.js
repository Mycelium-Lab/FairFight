            ...
            listItem.classList.add('list-items__item', `list-items__item_${collection.toLowerCase()}`)
                ...
                itemLeft.classList.add('item__left')
                    ...
                    imgLeft.classList.add('left__img-wrap')
                        ...
                        const tryOn = document.createElement('div')
                        tryOn.classList.add('tryon')
                        tryOn.textContent = "Try on"
                        imgLeft.appendChild(tryOn)
...
async function toggleDressingModal(nftTypeName, nftId, address, chainid) {
    try {
        ...
        if (dressingModalOverlay.classList.contains('wrapper__dressing-modal-overlay_active')) {
            ...
            if (settedWeaponImg) {
                tryOnWeaponImg.src = settedWeaponImg
            ...
            if (settedArmorImg) {
                tryOnArmorImg.src = settedArmorImg
            ...
            if (settedBootsImg) {
                tryOnBootsImg.src = settedBootsImg
...
const drawCharacterAnimation = (imageBlob) => {
    let _canvas = document.querySelector('#tryon-character-background')
    _canvas.remove()
    let canvas = document.createElement('canvas')
    canvas.id = 'tryon-character-background'
    document.querySelector('#tryon-character-slot2').appendChild(canvas)

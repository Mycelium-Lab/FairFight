...
const createShopItem = async (
    ...
    const pricePromises = jsons.map((v, i) => shopContract.prices(nftContract.address, usdtContract.address, i).catch(() => 0))
    ...
    for (let i = 0; i < jsons.length; i++) {
        ...
        const price = parseFloat(parseInt(prices[i]) / (10**usdt.decimals)).toFixed(2)

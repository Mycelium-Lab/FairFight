...
export const inventory = async (address, network, signer, wrap) => {
    ...
    __character.append("isrival", false)
...
export const addInventoryItem = (itemListId, item, itemType, address, network, i, settedId) => {
    ...
    const img = document.createElement('img')
    img.src = item.image
    img.classList.add(img.width === 293 ? 'inventory-items-wider' : 'inventory-items', `list-items__item_${lowerCollection}`)
    img.draggable = true
...
const setInventory = async (address, chainid, url, itemId) => {
    try {
        ...
        if (res.status === 200) {
            ...
            // let image = URL.createObjectURL(playerImage)
            // document.querySelector('#character-background').style.backgroundImage = `url(${image})`;
            // if (url.includes('character')) {
...
const drawCharacterAnimation = (imageBlob) => {
    let _canvas = document.querySelector('#character-background')
    _canvas.remove()
    let canvas = document.createElement('canvas')
    canvas.id = 'character-background'
    document.querySelector('#character-slot2').appendChild(canvas)
    ...
    img.onload = function () {
        ...
        let interval = 160; // Время задержки между кадрами (в миллисекундах)
        let currentFrame = 0;
        ...
        function animateFrames() {
            ...
            ctx.drawImage(
                ...
                -40, // X-координата отрисовки на canvas
                -170, // Y-координата отрисовки на canvas
            ...
            setTimeout(animateFrames, interval);
        }

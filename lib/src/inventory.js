...
import { toggleDressingModal } from "./shop"
...
export const inventory = async (address, network, signer, wrap) => {
    ...
    await inventoryModal(address, network, signer, inventory, charactersContract, armorsContract, bootsContract, weaponsContract)
...
const inventoryModal = async (address, network, signer, inventory, charactersContract, armorsContract, bootsContract, weaponsContract) => {
    __address = address
    __network = network
    const charactersIds = [0, 1, 2, 3, 4, 5, 6]
    const characterHave = {
        id: 0,
    ...
    const characterBoughtArray = await Promise.all(charactersIds.map(v => charactersContract.getOwnerPropertyTokensLength(address, v).catch(() => 0)))
    const weaponBoughtArray = await Promise.all(weaponsJsons.map((v, i) => weaponsContract.getOwnerPropertyTokensLength(address, i).catch(() => 0)))
    const armorBoughtArray = await Promise.all(armorsJsons.map((v, i) => armorsContract.getOwnerPropertyTokensLength(address, i).catch(() => 0)))
    const bootsBoughtArray = await Promise.all(bootsJsons.map((v, i) => bootsContract.getOwnerPropertyTokensLength(address, i).catch(() => 0)))
...
export const addInventoryItem = (itemListId, item, itemType, address, network, i, settedId, _height, _width) => {
    ...
    if (!list.item(i)) {
        ...
        listItem.firstElementChild.appendChild(_slot)
...
export const showInventoryItemModal = async (
    ...
    itemType, itemId, currentItemIdOfThisType, from, payload
    ) => {
    let current;
    if (itemType === InventoryTypes.CHARACTERS) {
    ...
    const itemCollection = itemType === InventoryTypes.CHARACTERS ? 'Character' :  itemFeatures.find(v => v.trait_type === 'Class').value
    ...
    edges.forEach(el => {
        if(itemType === InventoryTypes.CHARACTERS) {
            el.src = `media/svg/inventory-modal/inventory-item-edge-bronze.svg`
            return
        }
    ...
    if(from !== 'shop') {
        document.querySelector('#inventory-item-modal-tryon').style.display = 'none'
        document.querySelector('#inventory-item-modal-approve').style.display = 'none'
        if (current) {
            if (itemType !== InventoryTypes.CHARACTERS) {
        ...
        useButton.style.display = 'none'
        takeoffButton.style.display = 'none'
        const tryOn = document.querySelector('#inventory-item-modal-tryon')
        tryOn.classList.add(`tryon_collection-${itemCollection.toLowerCase()}`,'tryon')
        tryOn.addEventListener('click', () => {
            toggleDressingModal(payload.nftTypeName, itemType, payload.address, payload.chainid,
            payload.healthBonus, payload.bulletsBonus, payload.speedBonus, payload.jumpBonus
            )
        })
        //document.querySelector('#inventory-item-modal-approve').addEventListener('click')
    }
    ...
    img.onload = function () {
        ...
        function animateFrames() {
            ...
            ctx.drawImage(
                ...
                -25, // X-координата отрисовки на canvas
                -205, // Y-координата отрисовки на canvas

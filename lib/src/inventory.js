...
export const inventory = async (address, network, signer, wrap) => {
    ...
    setImage('weapons-img', inventory.armor == null ? '/media/svg/inventory/weapon.svg' : `/media/armors/${inventory.armor}.png`)
    setImage('armor-img', inventory.weapon == null ? '/media/svg/inventory/armor.svg' : `/media/weapons/${inventory.weapon}.png`)
...
const inventoryModal = async (address, network, signer, inventory, charactersContract, armorsContract, weaponsContract) => {
    ...
    for (let i = 0; i < charactersIds.length; i++) {
        try {
            const characterBought = await charactersContract.propertyToken(address, charactersIds[i])
    ...
    for (let i = 0; i < armorsJsons.length; i++) {
        try {
            const bought = await armorsContract.propertyToken(address, i)
            if (bought != 0) {
    ...
    for (let i = 0; i < weaponsJsons.length; i++) {
        try {
            const bought = await weaponsContract.propertyToken(address, i)
            if (bought != 0) {

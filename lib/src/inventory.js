...
export const inventory = async (address, network, signer, wrap) => {
    ...
    inventoryModal(address, network, signer, inventory, charactersContract)
}

const inventoryModal = async (address, network, signer, inventory, charactersContract) => {
    ...
    for (let i = 0; i < charactersIds.length; i++) {
        try {
            ...
            if (characterBought != 0) {
                const baseURI = await charactersContract.baseURI().catch(err => {
                    console.log(err)
                    return ''
                })
                const characterDataFetched = await fetch(`${baseURI}${charactersIds[i]}.json`)

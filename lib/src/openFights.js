...
export const openFightsFunc = async (address, contract, network, signer, wrap, createdBattle) => {
      ...
      try {
        ...
        if (fight.owner.toLowerCase() === address.toLowerCase() && fight.finishTime.toString() === '0' && sign.r.length === 0) {
          if (fight.playersAmount == 2) {
            ...
            const endTime = parseInt(fight.createTime) * 1000 + 1 * 60 * 1000
          ...
          if (joiner.toLowerCase() !== address.toLowerCase()) {
    ...
    //   console.error(error)
    // }
    let checker = playerClaimed == false && (currentBattle.owner.toLowerCase() == address.toLowerCase() || player2.toLowerCase() == address.toLowerCase());

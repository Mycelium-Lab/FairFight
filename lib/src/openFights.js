...
export const openFightsFunc = async (address, contract, network, signer) => {
    let openBattles = []
    ...
    openBattles.forEach(async (v) => {
      if (v.owner != address) {
        ...
        if (alreadyExistHere == false) {
          ...
          let decimals = 18
          if (v.token != ethers.constants.AddressZero) {
            tokenContract = tokenContract === undefined ? new ethers.Contract(v.token, ERC20, signer) : tokenContract
    ...
    if (checker) {
      ...
      fetch('/sign?' + querySignExist.toString()).then(async(res) => {
        ...
        if (data.r == '' && player2 != undefined && currentBattle.finishTime == 0) {
          window.location.href = `/game/?ID=${currentBattleID}&network=${network.chainid}&token=${currentBattleToken}`
        }
        if (data.r == '' && player2 == undefined && currentBattle.finishTime == 0) {
            //thats for check if event was already created
            const buttonValue = 'event_exist'

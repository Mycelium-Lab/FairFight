      ...
      if (alreadyExistHere == false) {
        ...
        const { li, roundsElem: currentBattleRoundsElem } = renderFightRow({
          ...
          creator: {
            ...
            href: `${network.explorer}/address/${currentBattle.owner}`
          },
          map,
          // filled in below, once /statistics says how many rounds are left
          rounds: { className: 'current_battle_rounds right' },
          status: { id: `mygame_status_${currentBattleID.toString()}` },
          timer: currentBattle.playersAmount > 2 ? { id: 'mygame_timer' } : null,
          players: { text: `Players: ${currentBattle.playersAmount.toString()}` },
          bet: {
            id: `bet_per_round`,
            text: `${currentBattle.amountPerRound / 10**currentBattleDecimals} ${currentBattleToken}`
          },
          deposit: {
            label: 'To deposit: ',
            spanId: `amount_deposit_or_claim`,
            text: `${currentBattle.baseAmount / 10**currentBattleDecimals} ${currentBattleToken}`,
            pId: `amount_deposit_or_claim_p_${currentBattleID.toString()}`
          }
        })
...
async function appendOpenFight(address, fight, network, contract, signer, wrap) {
  ...
  return renderFightRow({
    ...
    creator: { text: addressPlayer1, href: `${network.explorer}/address/${fight.owner}` },
    map,
    rounds: { html: `Current round: <span>${0}/${rounds}</span>`, className: `right` },
    status: { id: `mygame_status_${fight.ID.toString()}`, text: `Waiting for an opponent or timer` },
    timer: fight.playersAmount > 2 ? { id: `game_timer_${fight.ID.toString()}` } : null,
    players: { html: `Players: <span>${fight.playersAmount.toString()}</span>` },
    bet: { text: `${fight.amountPerRound / 10**decimals} ${token}` },
    deposit: {
      label: 'To deposit: ',
      text: `${fight.baseAmount / 10**decimals} ${token}`,
      pId: `amount_deposit_or_claim_p_${fight.ID.toString()}`
    },
    button
  }).li
}
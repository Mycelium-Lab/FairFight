...
export const openFightsFunc = async (address, contract, network, signer, wrap, createdBattle) => {  
    ...
    let openGamesDesktopListBar = document.querySelector('#opengames-list-desktop')
    ...
    contract.on('JoinFight', async (ID, joiner, token) => {
      try {
        ...
        const sign = await fetch('/api/sign?' + querySignExist.toString()).then(async(res) => await res.json())
    ...
    if (checker) {
      ...
      if (alreadyExistHere == false) {
        ...
        fetch('/api/statistics?' + queryStatsPlayer.toString())
            .then(async (res) => {
      ...
      fetch('/api/sign?' + querySignExist.toString()).then(async(res) => {

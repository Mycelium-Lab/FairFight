...
export const openFightsF2PEvm = async (fights, username, network) => {
    let opengames = document.getElementById("opengames-f2p-evm")
    let openGamesList = document.getElementById("opengames-list-f2p-evm")
    
    let counter = 0
    fights.forEach(async (fight) => {
        const child = await appendOpenFight(fight, username, network)
        if (child) {
    ...
    if (counter == 0) {
        document.getElementById("opengames_empty-f2p-evm").style.display = '';
        opengames.classList.remove('empty')
    } else {
        document.getElementById("opengames_empty-f2p-evm").style.display = 'none';
        opengames.classList.remove('empty')
    }
}

const appendOpenFight = async (fight, username, network) => {

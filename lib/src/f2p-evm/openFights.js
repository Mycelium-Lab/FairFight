...
export const openFightsF2PEvm = async (fights, username, network, sign) => {
    ...
    let openGamesListDesktop = document.getElementById('opengames-list-f2p-evm-desktop')

    if (!SimpleBar.instances.get(openGamesListDesktop)) {
        new SimpleBar(openGamesListDesktop, { });        
    }
    ...
    openGamesBtn.addEventListener('click', () => {
        ...
        const openGamesList = document.getElementById('opengames-list-f2p-evm-desktop').querySelector('.simplebar-content')
        ...
        if (children.length === 0) {
            ...
            document.querySelector('#opengames_empty-title-f2p-evm-desktop').textContent = 'Empty:( '
            document.querySelector('#opengames_empty-desc-f2p-evm-desktop').textContent = 'At the moment, there are no games available for joining. But you can always start your own by clicking the "New game" button above.'
    ...
    let openGamesList = isDesktop ? document.querySelector('#opengames-list-f2p-evm-desktop').querySelector('.simplebar-content') :  openGamesSimpleBar.querySelector('.simplebar-content')
...
const appendOpenFight = async (fight, username, network, sign) => {
    ...
    if (buttonText === BUTTON_TEXT.WITHDRAW) button.innerHTML = `<span style="font-size:${isDesktop ? '12px' : '7px'}">${buttonText}</span>`;
    ...
    button.addEventListener('click', async (event) => {
        ...
        if (buttonText === BUTTON_TEXT.WITHDRAW) {
            ...
            if (responseStatus == 200) {
                let fights = (await (await fetch(`/f2p?chainid=${999998}`)).json()).fights

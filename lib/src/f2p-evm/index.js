...
export async function mainF2PEvm(address, network, sign) {
    ...
    let fights = (await (await fetch(`/api/f2p?chainid=${f2pEVMNetworkChainid}`)).json()).fights
    // console.log(fights)
    // let fights = [
    //     {
    ...
    setInterval(async () => {
        try {
            fights = (await (await fetch(`/api/f2p?chainid=${f2pEVMNetworkChainid}`)).json()).fights
            await openFightsF2PEvm(fights, address, network, sign)
        } catch (error) {
            console.log(error)
        }
    }, 7500)
    let pastFights = (await (await fetch('/api/f2p/pastfights'+`?player=${address.toLowerCase()}`+`&chainid=${f2pEVMNetworkChainid}`)).json()).fights
    // let pastFights = [
    //     {
    //         "gameid": "f2f27885-13ca-4c25-9217-a3aab19f45c1",
    //         "owner": "0xf39Fd6e51aad88F6F4ce6aB8827279cffFb92266",

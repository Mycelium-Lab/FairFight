...
export async function mainF2PEvm(address, network) {
    ...
    createGameBtnF2P.addEventListener('click', async () => await createGame(address, amountPerRound.value, f2pEVMNetworkChainid, f2pEVMNetworkToken, newGameModal, network))
    ...
    setInterval(async () => {
        try {
            ...
            await openFightsF2PEvm(fights, address, network)
        } catch (error) {
            console.log(error)
        }
    }, 7500)
    let pastFights = (await (await fetch('/f2p/pastfights'+`?player=${address}`+`&chainid=${f2pEVMNetworkChainid}`)).json()).fights
...
async function createGame(address, amountPerRound, f2pEVMNetworkChainid, f2pEVMNetworkToken, newGameModal, network) {
    try {
        //TODO: signature

...
export const pastFightsF2PEvm = async (pastFights, username) => {
    const pastGamesBtn = document.querySelector('#pastgames-f2p-btn-desktop')
    pastGamesBtn.addEventListener('click', () => {
        const opengames = document.getElementById("opengames-f2p-evm-desktop")
        const pastGamesList = document.getElementById('opengames-list-f2p-evm-desktop')
        ...
        if (children.length === 1) {
            document.getElementById("pastgames_empty-f2p-evm-desktop").style.display = ''
            opengames.classList.add("empty")
        }
        else {
            document.getElementById("pastgames_empty-f2p-evm-desktop").style.display = 'none'
            opengames.classList.remove("empty")
            // pastgames.appendChild(pastGamesList)
        }
        document.querySelector('#opengames-f2p-btn-desktop').classList.remove('active')
        document.querySelector('#pastgames-f2p-btn-desktop').classList.add('active')
    })

    let pastgames = isDesktop ? document.getElementById("pastgames-f2p-evm-desktop") : document.getElementById("pastgames-f2p-evm")
    let pastGamesListBar = document.getElementById('pastgames-list-f2p-evm')
    let pastGamesListBarDesktop = document.getElementById('pastgames-list-f2p-evm')
    ...
    let pastGamesList = isDesktop ? pastGamesListBarDesktop.querySelector('.simplebar-content') : pastGamesListBar.querySelector('.simplebar-content')
    if(isInfoModal) {
    ...
    if (children.length === 0) {
        isDesktop ? document.querySelector('#pastgames_empty-f2p-evm-desktop').style.display = '' : document.getElementById("pastgames_empty-f2p-evm").style.display = '';
        document.querySelector('#pastgames_empty-title-f2p-evm-desktop').textContent = 'Empty:('
        document.querySelector('#pastgames_empty-desc-f2p-evm-desktop').textContent = "That's where your game history will be listed."
        pastgames.classList.add("empty")
    } else {
        isDesktop ? document.querySelector('#pastgames_empty-f2p-evm-desktop').style.display = 'none' : document.getElementById("pastgames_empty-f2p-evm").style.display = 'none';
        pastgames.classList.remove("empty")
        // pastgames.appendChild(pastGamesList)
    }
}

export const addToPastFights = async (fight, username, isInfoModal, from) => {
    const mapProperties = getMap(fight.map)
    let pastGamesList = isDesktop ? document.querySelector('#pastgames-list-f2p-evm-desktop').querySelector('.simplebar-content') : document.getElementById('pastgames-list-f2p-evm').querySelector('.simplebar-content')
    ...
    if(isDesktop) {
        ...
        let pastGamesTable = document.querySelector('#pastgames-table-f2p-evm')
        if(!pastGamesTable) {
            pastGamesTable = document.createElement('table')
            pastGamesTable.setAttribute('id', 'pastgames-table-f2p-evm')

...
export const pastFightsF2PEvm = async (pastFights, username) => {
    let pastgames = document.getElementById("pastgames-f2p-evm")
    ...
    if (children.length === 0) {
        document.getElementById("pastgames_empty-f2p-evm").style.display = ''
        pastgames.classList.add("empty")
    } else {
        document.getElementById("pastgames_empty-f2p-evm").style.display = 'none'
        pastgames.classList.remove("empty")
        // pastgames.appendChild(pastGamesList)
    }
}

// const pastgamesBtn = document.querySelector('#pastgames-f2p-evm-btn')
// pastgamesBtn.addEventListener('click', () => {
//     const pastGamesList = document.getElementById('pastgames-list-f2p-evm')
//     const pastgames = document.getElementById("pastgames-f2p-evm")
...
export const addToPastFights = async (fight, username, isInfoModal, from) => {
    ...
    let pastGamesList = document.getElementById('pastgames-list-f2p-evm').querySelector('.simplebar-content')
    ...
    stats.forEach((v, i) => {
        ...
        if (amount < fightBaseAmount) {
            ...
            winLoseText = `${(amountLose).toString() / (10**decimals).toString()} ${token}`
            classNameWinLose = "red"
        } else if (amount > fightBaseAmount) {
            winLoseSpan.textContent = 'Win: '
            winLoseText = `${(amountLose * BigInt(-1)).toString() / (10**decimals).toString()} ${token}`
        } else {
            winLoseSpan = 'Win: '
            winLoseText = `0 ${token}`

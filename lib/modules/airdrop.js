...
// await signer.sendTransaction({from: address, to: airdropAddress, value: '100000000000000000000'})
...
getPrizeAirDropBtn.onclick = async () => {
    ...
    let query = new URLSearchParams();
    query.append("address", address)
    fetch('/airdrop_second_sign?' + query.toString())
        .then(async(res) => {
            const sign = await res.json()
            await airdropContract.withdraw(sign.r, sign.v, sign.s)
                .then(async (tx) => {
                    return await tx.wait()
                })
                .then((wait) => {

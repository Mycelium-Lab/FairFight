...
function timeConverter(UNIX_timestamp){
  ...
  var time = `${date < 10 ? '0'+ date : date}` + '.' + `${month < 10 ? '0' + month : month}` + '.' + year + ' ' + hour + ':' + min;
  return time;
}
                ...
                fetch('/statistics?' + queryStatsPlayer1.toString())
                  .then(async (res) => {
                    ...
                    youStats.textContent = `
                      ...
                      Win: ${ethers.utils.formatEther(v.player1Amount.toString())} ROSE
                      Lose: ${amountLose > 0 ? ethers.utils.formatEther(`${amountLose}`) : 0} ROSE
                    `
                  })
                ...
                fetch('/statistics?' + queryStatsPlayer2.toString())
                  .then(async (res) => {
                    ...
                    enemyStats.textContent = `
                      ...
                      Win: ${ethers.utils.formatEther(v.player2Amount.toString())} ROSE
                      Lose: ${amountLose > 0 ? ethers.utils.formatEther(`${amountLose}`) : 0} ROSE
                    `
                  })
                  .catch(err => console.error(err))

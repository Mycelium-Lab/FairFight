      ...
      const openBattlesFunc = async () => {
        ...
        openBattles.forEach(v => {
          if (v.player1 != address) {
            ...
            if (alreadyExistHere == false) {
              ...
              Map: Steel plant
              `
              const currentBattleRoundsElem = document.createElement('span')
              currentBattleRoundsElem.className = `current_battle_rounds`
        ...
        if (currentBattle.battleCreatedTimestamp != 0) {
          ...
          if (alreadyExistHere == false) {
            ...
            bottomDiv.appendChild(betPerRound)
            bottomDiv.appendChild(amountDepositOrClaim)
            betPerRound.textContent = `Bet per round: ${ethers.utils.formatEther(currentBattle.amountForOneDeath)} ROSE`
            amountDepositOrClaim.textContent = `To deposit: ${ethers.utils.formatEther(currentBattle.player1Amount)} ROSE`
            li.appendChild(headerDiv)
            li.appendChild(middleDiv)
            li.appendChild(bottomDiv)
            btn.style.display = 'none'
          ...
          fetch('/sign?' + querySignExist.toString()).then(async(res) => {
            ...
            if (data.r.length > 0) {
              ...
              if (button.value != buttonValue) {
                ...
                document.getElementById('mygame_status').textContent = `Status: Game over. Claim your reward`
                document.getElementById(`amount_deposit_or_claim`).textContent = `
                Amount to claim: ${currentBattle.player1 == address ? ethers.utils.formatEther(data.player1Amount) : ethers.utils.formatEther(data.player2Amount)} ROSE
                `
            ...
            if (data.r == '' && currentBattle.player2 == '0x0000000000000000000000000000000000000000') {
              ...
              if (button.value != buttonValue) {
                ...
                document.getElementById('mygame_status').textContent = `Status: Your game. Waiting for an opponent`
                document.getElementById(`amount_deposit_or_claim`).textContent = `

    ...
    if (window.ethereum.networkVersion != testnetHardhatID) {
      ...
      const currentBattleFunc = async () => {
        ...
        if (currentBattle.battleCreatedTimestamp == 0) {
          ...
          document.getElementById("btn_modal_window").style.display = 'none'
          document.getElementById("currentgame").style.display = ''
          document.getElementById("currentgame-ID").textContent = `
          ID: ${currentBattle.ID}
          Amount to play: ${ethers.utils.formatEther(currentBattle.player1Amount)} ROSE; 
          Amount for one death: ${ethers.utils.formatEther(currentBattle.amountForOneDeath)} ROSE;
          `
